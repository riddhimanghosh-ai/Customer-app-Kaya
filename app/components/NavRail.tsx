  ...
  return (
    <div className="nav-rail">
      ...
      <Section label="Care" open={true} onToggle={() => {}}>
        <NavItem icon={IconHome} label="Overview" active={active === 'dashboard'} onClick={() => router.push('/dashboard')} />
        <NavItem icon={IconAppt} label="Appointments" badge="2" active={active === 'appointments'} onClick={() => router.push('/sessions')} />
        <NavItem icon={IconMed} label="Medications" badge="4" active={active === 'prescriptions'} onClick={() => router.push('/prescriptions')} />
        <NavItem icon={IconProgress} label="Progress" active={active === 'before-after'} onClick={() => router.push('/before-after')} />
        <NavItem icon={IconSummary} label="Summaries" active={active === 'summary'} onClick={() => router.push('/summary')} />
        <NavItem icon={IconChat} label="Dr. AI" active={active === 'chatbot'} onClick={() => router.push('/chatbot')} />
      </Section>

      <Section label="Membership" open={true} onToggle={() => {}}>
        <NavItem icon={IconRewards} label="Loyalty" active={active === 'loyalty'} onClick={() => router.push('/loyalty')} />
        <NavItem icon={IconRefer} label="Referrals" active={active === 'referral'} onClick={() => router.push('/referral')} />
        <NavItem icon={IconPackage} label="Products" active={active === 'products'} onClick={() => router.push('/products')} />
      </Section>

      <Section label="Learn" open={true} onToggle={() => {}}>
        <NavItem icon={IconBlog} label="Articles" active={active === 'blogs'} onClick={() => router.push('/blog')} />
        <NavItem icon={IconVideo} label="Videos" active={active === 'videos'} onClick={() => router.push('/videos')} />
      </Section>
